...
export default function SuperResolutionPage() {
  ...
  const [superRes, setSuperRes] = useState("");
  const [isLoading, setIsLoading] = useState(false);

  const handleSubmit = async () => {
    if (!selectedFile || superRes.trim() === "") return;

    const formData = new FormData();
    formData.append('image', selectedFile);
    formData.append('superRes', superRes);
  ...
  return (
    <div className="min-h-screen px-6 py-12 bg-neutral-50 text-gray-800">
      <div className="max-w-5xl mx-auto space-y-10">
        <h1 className="text-5xl font-bold text-center text-gray-700 tracking-tight">Super-Resolution</h1>
          ...
          <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
            <CardContent className="p-6 space-y-4">
              ...
              {outputImage && !isLoading && selectedFile && (
                <a
                  href={outputImage}
                  download={`${selectedFile.name.replace(/\.[^/.]+$/, '')}_object_removed_${superRes}.jpg`}
                >
        ...
        <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 space-y-6">
            <div className="space-y-2">
              <label className="font-semibold text-sm text-gray-700">Content of the Image</label>
              <Textarea
                value={superRes}
                onChange={(e) => setSuperRes(e.target.value)}
                placeholder="Type the content of the image you want to upscale for better results."
            ...
            <Button
              ...
              disabled={isLoading || superRes.trim() === ""}
            ...
            >
              {isLoading ? (
                ...
                <>
                  <UploadCloud className="mr-2 w-4 h-4" /> Upscale
                </>
              )}
            </Button>
          </CardContent>
