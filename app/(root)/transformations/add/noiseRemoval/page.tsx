...
export default function NoiseRemovalPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/denoise-image', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen px-6 py-12 bg-neutral-50 text-gray-800">
      <div className="max-w-5xl mx-auto space-y-10">
        <h1 className="text-5xl font-bold text-center text-gray-700 tracking-tight">Noise Removal</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
            <CardContent className="p-6 space-y-4">
              <h2 className="text-xl font-semibold text-indigo-600">Denoised Output</h2>
              <div className="w-full aspect-square border-2 border-dashed border-indigo-300 rounded-lg flex items-center justify-center bg-white relative overflow-hidden">
                {isLoading ? (
                  <div className="flex flex-col items-center justify-center animate-pulse">
                    ...
                    <span className="text-gray-500 text-sm mt-2">Removing noise...</span>
                  </div>
                ) : outputImage ? (
        ...
        <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 space-y-6">
            ...
            >
              {isLoading ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Removing Noise...
                </>
              ) : (
                <>
                  <UploadCloud className="mr-2 w-4 h-4" /> Remove Noise
                </>
              )}
            </Button>
          </CardContent>
