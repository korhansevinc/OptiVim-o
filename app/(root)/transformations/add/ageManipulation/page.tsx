...
export default function AgeManipulationPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/age-manipulation', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen px-6 py-12 bg-neutral-50 text-gray-800">
      <div className="max-w-5xl mx-auto space-y-10">
        <h1 className="text-5xl font-bold text-center text-gray-700 tracking-tight">Age Manipulation</h1>
          ...
          <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
            <CardContent className="p-6 space-y-4">
              ...
              {outputImage && !isLoading && selectedFile && (
                <a
                  href={outputImage}
                  download={`${selectedFile.name}_target_age_${targetAge}.jpg`}
                >
        ...
        <Card className="shadow-md bg-white border border-gray-200 hover:shadow-lg transition-shadow">
          <CardContent className="p-6 space-y-6">
            <div className="space-y-2">
              <Label className="font-medium text-gray-700">Target Age: {targetAge}</Label>
              <Slider
                min={0}
                max={100}
                step={1}
